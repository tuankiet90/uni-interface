...
export default function usePermit2Allowance(
  ...
  const { tokenAllowance, isSyncing: isApprovalSyncing } = useTokenAllowance(token, account, GET_PERMIT2_ADDRESS())
  const updateTokenAllowance = useUpdateTokenAllowance(amount, GET_PERMIT2_ADDRESS())
  const revokeTokenAllowance = useRevokeTokenAllowance(token, GET_PERMIT2_ADDRESS())
  ...
  const isApprovalPending = useHasPendingApproval(token, GET_PERMIT2_ADDRESS())
  const isRevocationPending = useHasPendingRevocation(token, GET_PERMIT2_ADDRESS())
