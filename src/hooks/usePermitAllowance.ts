...
export function usePermitAllowance(token?: Token, owner?: string, spender?: string) {
  const contract = useContract<Permit2>(GET_PERMIT2_ADDRESS(), PERMIT2_ABI)
...
export function useUpdatePermitAllowance(
  ...
  return useCallback(async () => {
    try {
      ...
      const { domain, types, values } = AllowanceTransfer.getPermitData(permit, GET_PERMIT2_ADDRESS(), chainId)
