...
export async function getApproveInfo(
  ...
  try {
    const allowance = await tokenContract.callStatic.allowance(account, GET_PERMIT2_ADDRESS())
  ...
  try {
    const approveTx = await tokenContract.populateTransaction.approve(GET_PERMIT2_ADDRESS(), MaxUint256)
